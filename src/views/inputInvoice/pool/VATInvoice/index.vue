<template>
  <div class="">
    <el-card shadow="never" body-style="padding: 15px 15px 0;">
    ...
    <el-card shadow="never" style="margin-top: 10px;" body-style="padding: 15px 15px 0;">
      <article>
        <article style="padding: 0 0 15px">
          <el-row>
            ...
            <el-col :span="16" style="text-align: right">
              <el-button-group>
                <el-button type="" :disabled="isSelected.length <= 0" @click="handleRequire">发票查验</el-button>
                <el-button type="" :disabled="isSelected.length <= 0" @click="handleEditeStatus">修改待认证科目</el-button>
                <el-button type="" :disabled="isSelected.length <= 0" @click="handleEnterAccount(1)">发票入账</el-button>
                <el-button type="" :disabled="isSelected.length <= 0" @click="handleEnterAccount(2)">撤销入账</el-button>
                <el-button type="" :disabled="isSelected.length <= 0" @click="handleStatus(1)">确认收票</el-button>
                <el-button type="" :disabled="isSelected.length <= 0" @click="handleStatus(2)">撤销收票</el-button>
                <el-button type="" @click="handleExportRange">导出查询结果</el-button>
                <el-button type="" :disabled="isSelected.length <= 0" @click="handleExportSelected">导出选中发票</el-button>
              </el-button-group>
        ...
        <article style="">
          <el-table :data="tableData" :border="true" @row-click="handleRowClick" highlight-current-row
            :row-class-name="rowClassName" @selection-change="handleSelectionChange" ref="tableRef" :height="height"
            ...
            <el-table-column prop="zzfphm" label="发票号码" minWidth="160">
            </el-table-column>
            ...
            <el-table-column prop="aciton" fixed="right" label="操作" width="120" :header-align="'center'"
              :align="'center'">
              <template slot-scope="scope">
        ...
        <article class="table_bottom_page">
          ...
          <article>
            <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
              :current-page="page.currentPage" :page-sizes="page.sizes" :page-size="page.pageSize"
    ...
    <el-card shadow="never" class="bottom_crad" style="margin-top: 10px;" body-style="padding: 15px 15px 0;">
      <article>
        <el-table ref="bottomTableRef" :data="bottomTableData" :border="true" v-loading="loading_1" height="150px">
          <el-table-column type="index" width="55" label="序号" align="center">
          </el-table-column>
          <el-table-column prop="xmmc" label="项目名称" minWidth="180" :show-overflow-tooltip="true" align="center">
          </el-table-column>
          <el-table-column prop="ggxh" label="规格型号" minWidth="140" align="center">
          </el-table-column>
          <el-table-column prop="dw" label="单位" minWidth="120" align="center">
          </el-table-column>
          <el-table-column prop="fpjysl" label="数量" minWidth="120" align="center">
          </el-table-column>
          <el-table-column prop="fpjydj" label="单价" minWidth="120" align="center">
          </el-table-column>
          <el-table-column prop="je" label="金额" minWidth="140" align="center">
          </el-table-column>
          <el-table-column prop="slv" label="增值税税率/征收率" minWidth="160" align="center">
          </el-table-column>
          <el-table-column prop="se" label="税额" minWidth="120" align="center">
          </el-table-column>
          <el-table-column prop="hsje" label="含税金额" minWidth="110" align="center">
          </el-table-column>
          <el-table-column prop="kce" label="扣除额" minWidth="110" align="center">
          </el-table-column>
          <el-table-column prop="sphfwssflhbbm" label="商品和服务税收分类合并编码" width="180" align="center">
          </el-table-column>
        </el-table>
      </article>
      <article style="padding: 8px 0;">
        <el-pagination @size-change="handleBottomSizeChange" @current-change="handleBottomCurrentChange"
          :current-page="page_bottom.currentPage" :page-sizes="page_bottom.pageSizes" :page-size="page_bottom.pageSize"
          layout="total, prev, pager, next, jumper" :total="bottom_total">
        </el-pagination>
      </article>
    </el-card>
    <lg-collect-ticket-mage v-if="dialog.statusVisible" :visible.sync="dialog.statusVisible" width="50%"
      :title="dialog.statusTitle" :row-data="rowData" :type-status="typeStatus"
      @successDone="handleDone"></lg-collect-ticket-mage>
    <lg-enter-account-mage v-if="dialog.enterVisible" :visible.sync="dialog.enterVisible" width="40%"
      :title="dialog.enterTitle" :row-data="rowData" :type-status="typeStatus"
      @successDone="handleDone"></lg-enter-account-mage>
    <!-- 待修改认证科目 -->
    <lg-edie-verified v-if="dialog.editeVisible" :visible.sync="dialog.editeVisible" width="50%" title="修改会计科目操作"
      :row-data="rowData" :type-status="typeStatus" @successDone="handleDone"></lg-edie-verified>
    <!-- 发票检验 -->
    <lg-invoice-require :visible.sync="dialog.requireVisbile" v-if="dialog.requireVisbile" title="发票检验" width="45%"
