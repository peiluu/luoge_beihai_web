<template>
  <div class="com-invoice">
    ...
    <div class="custom-table">
      ...
      <el-table row-key="id" :reserve-selection="true" ref="table" :data="data" :height="height" border stripe @row-click="rowClcik"
        :header-cell-style="{ fontWeight: 400, borderTop: '1px solid #adb4bc', background: '#f7f9fd', color: '#333333', padding: '7px 0' }" tooltip-effect="dark"
        :tree-props="{ children: 'children', hasChildren: 'hasChildren' }" v-loading="loading" :row-class-name="tableSelectedRow" @selection-change="handleSelection">
        ...
        <el-table-column prop="cljg" label="勾选状态" min-width="100">
          <template slot-scope="{row}"> {{ row.cljg == '01' ? '已勾选' : '未勾选' }} </template>
        </el-table-column>
        <el-table-column prop="purchaserstatus" label="入账状态" min-width="100">
          <template slot-scope="{row}"> {{ row.purchaserstatus ? purchaserstatusMap[row.purchaserstatus] : '' }} </template>
        </el-table-column>
        <el-table-column prop="preCheck" label="预勾选状态" min-width="100" v-if="isBusinessFormat">
          <template slot-scope="{row}"> {{ row.preCheck == 'Y' ? '已预勾选' : '未预勾选' }} </template>
        </el-table-column>
        ...
        <el-table-column prop="fplx" label="发票种类" min-width="180">
          <template slot-scope="{row}"> {{ inputFplxMap[row.fplx] }} </template>

        </el-table-column>

        <el-table-column prop="kprq" label="开票日期" min-width="120">
          <template slot-scope="{row}"> {{ row.kprq ? dateFormat('YYYY-MM-DD', row.kprq) : '' }} </template>
        </el-table-column>
        ...
        <el-table-column prop="accountPeriod" label="会计期间" min-width="120">
          <template slot-scope="{row}"> {{ row.accountPeriod ? dateFormat('YYYY-MM', row.accountPeriod) : '' }} </template>
        </el-table-column>
        ...
        <el-table-column prop="yxdkse" label="可抵扣税额" min-width="120" align="right">
          <template slot-scope="{row}"> {{ formatMoney(row.yxdkse) }} </template>
        </el-table-column>

        <el-table-column prop="fpzt" label="发票状态" min-width="120">
          <template slot-scope="{row}"> {{ row.fpzt ? fpztMap[row.fpzt] : '' }}</template>
        </el-table-column>

        <el-table-column prop="sfycpz" label="是否异常凭证" min-width="120">
          <template slot-scope="{row}"> {{ row.sfycpz == '01' ? '正常' : row.sfycpz == '02' ? '异常' : row.sfycpz == '03' ? '疑似异常' : '' }} </template>
        </el-table-column>

        <el-table-column prop="gxsj" label="勾选时间" min-width="120">
          <template slot-scope="{row}"> {{ row.gxsj ? dateFormat('YYYY-MM-DD', row.gxsj) : '' }} </template>
        </el-table-column>
        ...
        <el-table-column prop="accountTime" label="记账日期" min-width="120">
          <template slot-scope="{row}"> {{ row.accountTime ? dateFormat('YYYY-MM-DD', row.accountTime) : '' }} </template>
        </el-table-column>

        <el-table-column prop="reimburseDate" label="报销日期" min-width="120">
          <template slot-scope="{row}"> {{ row.reimburseDate ? dateFormat('YYYY-MM-DD', row.reimburseDate) : '' }} </template>
        </el-table-column>

        <el-table-column prop="sfycpz" label="是否入发票池" min-width="120">
          <template slot-scope="{row}">{{ row.syncInvoice == 'Y' ? '是' : '否' }}</template>
        </el-table-column>

      </el-table>
      <TableCounter ref="tableCounter" />
    ...
    <el-dialog title="请确认" :visible.sync="dialogVisible" width="40%" :before-close="handleClose">
      <div class="submit-info">
        ...
        <template v-else>
          <div class="title title-normal"><i class="el-icon-warning" />本次勾选<span>{{ selections.length }}</span>张发票，税额合计<span>{{ selecedInfo.hjse }}</span>元
          </div>
          <div class="tip">是否确认提交</div>
        </template>
      </div>
    ...
    <!-- 设置税负率 -->
    <el-dialog title="进销比例设置" :visible.sync="dialogVisiblePercent" width="40%" :before-close="handleClose" class="submit-dialog">
      <div class="dialog-main">请输入进销比例<el-input placeholder="请输入（0-100）%" v-model="lossTaxRate" />%</div>
      <span slot="footer">
...
export default {
    ...
    return {
      ...
      searchList: [
        ...
        },
        {
          ...
          val: '',
          type: "select",
          options: [{ value: "", label: "全部" }, { value: "Y", label: "已预勾选" }, { value: "N", label: "未预勾选" }],
        }],
      selecedInfo: {
        number: 0,
        hjje: 0,
  ...
  methods: {
    ...
    async submitRevokePreCheck(type) {
      ...
      this.$confirm(`您确定要${type === 'Y' ? '提交' : '撤销'}预勾选吗？`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        const ids = this.selections.map((item) => item.id)
        const { code = '' } = await submitRevokePreCheck({
          type,
          ids,
          gfsbh: this.nsrsbh,
    ...
    async exportInvoiceCheck() {
      ...
      await exportInvoiceCheck({
        reqData: {
          ...this.searchParam, sign: 'Y', pageNo: 1, pageSize: 9999999
        },
        fileName
      })
    },
