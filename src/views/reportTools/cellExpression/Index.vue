  ...
  <div>
    <div style="position: fixed;top:0;left:300px;">
      <div style="margin: 8px;font-size: 14px;">
        选择报表
        <el-select filterable  class="form-inline-input" size="small" v-model="searchParam.templateId" style="margin: 0 20px 0 5px;">
          <el-option v-for="option in templateOptions" :key="option.id" :label="option.templateName" v-model="option.id"></el-option>
        </el-select>

        所属业态
        <el-select clearable filterable  class="form-inline-input" size="small" v-model="searchParam.business" style="margin: 0 20px 0 5px;">
          <el-option v-for="option in businessOptions" :key="option" :label="option" :value="option"></el-option>
        </el-select>

        纳税主体
        <el-select clearable filterable  class="form-inline-input" size="small" v-model="searchParam.nsrsbh" style="margin: 0 20px 0 5px;">
          <el-option v-for="(option, index) in taxBodyOptions" :key="index" :label="option.nsrmc" v-model="option.nsrsbh"></el-option>
        </el-select>
    ...
    ></div>
    <!--配置公式-->
    <ExpModal :expCellExpression="expCellExpression" :expVisible="expVisible" @closeExp="closeExp" @submitExp="submitExp"></ExpModal>
    <!--配置动态行-->
    <DynamicRowModal :dynamicSettingId="dynamicSettingId" :currentCell="currentCell" :modalVisible="modalVisible" @closeModal="closeModal" @submitModal="submitModal"></DynamicRowModal>
    <!--配置下拉框-->
    <DropdownModal :expCellExpression="expCellExpression" :dropdownVisible="dropdownVisible" @closeDropdown="closeDropdown" @submitDropdown="submitExp"></DropdownModal>
    <!--基础配置-->
    <BasicModal :expCellExpression="expCellExpression" :basicVisible="basicVisible" @closeBasicModal="closeBasicModal" @submitBasicModal="submitExp"></BasicModal>
  </div>
</template>
