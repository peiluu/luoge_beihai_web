<template>
  <div class="main-content" :style="'height: ' + contentHeight + 'px;'">
    <form-list :columns="columns"
      ...
      <template #myscope="row">
        <el-popover placement="left" trigger="hover" popper-class="customPopper">
          <template>
            <el-button v-if="row.data.status==0" @click.stop="editDeclaration(row.data)" type="primary">编辑</el-button>
            <el-button v-if="row.data.status==0 || row.data.status==1" @click.stop="delDeclaration(row.data)" type="danger">删除</el-button>
            <el-button v-if="row.data.status==0 || row.data.status==1" @click.stop="fillDeclaration(row.data)" type="primary">填报</el-button>
            <el-button v-if="row.data.status==1"  @click.stop="submitDeclaration(row.data)" type="primary">提交</el-button>
            <el-button v-if="row.data.status==2 || row.data.status==3" @click.stop="toDeclareForm(row.data)" type="primary">查看</el-button>
            <el-button v-if="row.data.status==2" @click.stop="undoDeclaration(row.data)" type="primary">撤销</el-button>
            <el-button v-if="row.data.status==2" @click.stop="uploadDeclaration(row.data)" type="primary">上传完税凭证</el-button>
          </template>
      ...
      <el-form ref="dlgForm" :model="dlgForm" :rules="dlgRules" size="mini" inline>
        <el-form-item label="纳税主体" prop="nsrmc">
          <el-select size="small" filterable v-model="dlgForm.nsrmc" @change="taxBodyChange">
            <el-option v-for="option in taxBodyOptions" :key="option.id" :label="option.nsrmc + ' '+ option.nsrsbh" v-model="option.nsrmc"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item size="small" label="税款所属期" prop="declareDate">
          <QuarterDatePicker :key="dlgForm.period" :pickerType="dlgForm.period"  :defaultValue="dlgForm.declareDate" propsParam="declareDate" @getQuarterPickerFrom="getQuarterPickerFrom" ref="quarterDatePicker" />
        </el-form-item>
    ...
    <!--申报明细数据确认-->
    <DetailConfirmModal :detailConfirmVisible="detailConfirmVisible"
                        :curRow="curRow"
