...
export const addRedInvoice = data =>
  postJSON('/redInvoice/addRedInvoice', data, null, true);

/**
 * @desption 新增红字申请单 - 提交
 */
export const submitRedInvoice = data =>
  postJSON('/redInvoice/submitRedInvoice', data, null, true);

/**
 * @desption 蓝字发票详情查询 - 新增红字信息单入口
 */
export const queryBlueInvoiceById = data =>
  postJSON(`/blueInvoice/queryBlueInvoiceById/${data.id}`, data);

/**
 * @desption 蓝字发票详情查询 - 已开票红冲入口
 */
export const invoiceInfo = data =>
  postJSON(`/invoiceUpload/invoiceInfo/${data.id}`, data);
